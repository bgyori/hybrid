...
function model = createHeart()
	...
	model.modes(2).ode = @(t,x) ([ (1 - 0.0 - x(1)/EPI_TO2 - 0.0) ; ((0.94-x(2))/(EPI_TW1M + (EPI_TW2M - EPI_TW1M) * 0.5 * (1.+tanh(EPI_KWM*(x(1)-EPI_UWM))))) ; (-x(3)/EPI_TV2M) ; ((((1.+tanh(EPI_KS*(x(1)-EPI_US))) * 0.5) - x(4))/EPI_TS1) ; 1 ]);
	% Array of guards
	% Formula of the guard
	...
	% ODE definition, return column vector dx
	model.modes(7).ode = @(t,x) ([ (0 - 0.0 - x(1)/EPI_TO2 - 0.0) ; ((0.94-x(2))/(EPI_TW1M + (EPI_TW2M - EPI_TW1M) * 0.5 * (1.+tanh(EPI_KWM*(x(1)-EPI_UWM))))) ; (-x(3)/EPI_TV2M) ; ((((1.+tanh(EPI_KS*(x(1)-EPI_US))) * 0.5) - x(4))/EPI_TS1) ; 1 ]);
	% Array of guards
	% Formula of the guard
